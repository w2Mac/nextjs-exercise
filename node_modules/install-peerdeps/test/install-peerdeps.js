...
function getPackageData({
  ...
  const requestProxy = process.env.HTTP_PROXY || process.env.http_proxy || `${proxy}`;

  if (requestProxy !== "undefined") {
    options.agent = new _httpsProxyAgent.default(requestProxy);
  }

  return (0, _requestPromiseNative.default)(options).then(response => {
    const {
      statusCode
    } = response;
...
function spawnInstall(command, args) {
  return new Promise((resolve, reject) => {
    ...
    const installProcess = (0, _child_process.spawn)(command, args, {
      cwd: process.cwd(),
...
function getPackageJson({
  ...
  return getPackageData({
    ...
    const versions = Object.keys(data.versions); // Get max satisfying semver version

    let versionToInstall = (0, _semver.maxSatisfying)(versions, version); // If we didn't find a version, maybe it's a tag

    if (versionToInstall === null) {
      const tags = Object.keys(data["dist-tags"]); //  If it's not a valid tag, throw an error
...
function installPeerDeps({
  ...
  getPackageJson({
    ...
    auth,
    proxy
  }, onlyPeers, version) // Catch before .then because the .then is so long
  .catch(err => cb(err)).then(data => {
    ...
    Object.keys(peerDepsVersionMap).forEach(depName => {
      ...
      const peerDepVersion = peerDepsVersionMap[depName]; // Check if there is whitespace
      ...
      if (peerDepVersion.indexOf(" ") >= 0) {
        ...
        const rangeSplit = peerDepVersion.split(" ").map(v => (0, _semver.coerce)(v)).filter(v => (0, _semver.valid)(v));
        const versionToInstall = (0, _semver.maxSatisfying)(rangeSplit, peerDepVersion);
    ...
    let args = []; // If any proxy setting were passed then include the http proxy agent.
    ...
    if (dryRun) {
      ...
      console.log(commandString);
      spawnInstall(packageManager + extra, args).then(() => cb(null)).catch(err => cb(err));
    }
  });
} // Export for testing
