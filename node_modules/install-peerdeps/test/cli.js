...
program.version(version).description("Installs the specified package along with correct peerDeps.").option("-D", "Install the package as a devDependency (alias for `-d`)").option("-d, --dev", "Install the package as a devDependency").option("-g, --global", "Install the package globally").option("-o, --only-peers", "Install only peerDependencies of the package").option("-S, --silent", "If using npm, don't save in package.json").option("-Y, --yarn", "Install with Yarn").option("-P, --pnpm", "Install with pnpm").option("-r, --registry <uri>", "Install from custom registry (defaults to NPM registry)").option("--dry-run", "Do not install packages, but show the install command that will be run").option("-a, --auth <token>", "Provide an NPM authToken for private packages.").option("-p, --proxy <http_proxy>", "Enable http proxy to connect to the registry").option('-x, --extra-args "<extra_args>"', "Extra arguments to pass through to NPM or Yarn").usage("<package>[@<version>], default version is 'latest'").parse(process.argv); // Print program name and version (like what Yarn does)
...
if (program.registry) {
  const {
    registry
  } = program; // Check if last character in registry is a trailing slash
...
if ((0, _hasYarn.default)() && packageManager !== C.yarn && !program.silent) {
  ...
  (0, _promptly.confirm)("It seems as if you are using Yarn. Would you like to use Yarn for the installation? (y/n)", (err, value) => {
    if (err) {
    ...
    (0, _installPeerdeps.default)(_objectSpread({}, options, {
      packageManager
    }), installCb);
  });
} else {
