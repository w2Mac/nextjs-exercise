...
function spawnCli(extraArgs, cwd = "sandbox") {
  const fullCwd = _path.default.join(__dirname, "..", "fixtures", cwd); // Clean up sandbox before and after every run


  if (cwd === "sandbox") {
    _fs.default.copyFileSync(_path.default.join(fullCwd, "package-clean.json"), _path.default.join(fullCwd, "package.json"));
  }

  const cli = (0, _child_process.spawn)("node", [...["--require", "@babel/register", _path.default.join(__dirname, "cli.js")], ...extraArgs], {
    cwd: fullCwd
...
async function getCliInstallCommand(extraArgs) {
  return new Promise((resolve, reject) => {
    ...
    cli.on("close", () => {
      ...
      const lines = fullstdout.join("").split(/\r?\n/g).filter(l => !!l.trim());
...
(0, _tape.default)("errors when more than one package is provided", t => {
...
(0, _tape.default)("errors when no arguments are provided", t => {
  const cli = spawnCli([]);
  cli.on("exit", code => {
    t.notEqual(code, 0, `errored, exit code was ${code}`);
    t.end();
...
(0, _tape.default)("errors when the package name argument is formatted incorrectly", t => {
...
(0, _tape.default)("only installs peerDependencies when `--only-peers` is specified", async t => {
  t.plan(1);

  try {
    const command = await getCliInstallCommand(["eslint-config-airbnb", "--only-peers"]);
    t.equal(/ eslint-config-airbnb /.test(command), false);
  } catch (err) {
    t.fail(err);
  }
});
(0, _tape.default)("adds explicit `--save-dev` flag when using `-D, -d, --dev` with NPM", async t => {
  const flags = ["-D", "-d", "--dev"];
  t.plan(flags.length);

  try {
    const commands = await Promise.all(flags.map(flag => getCliInstallCommand(["eslint-config-airbnb", flag])));
    commands.forEach((command, i) => t.equal(/ --save-dev /.test(command), true, `flag: \`${flags[i]}\``));
  } catch (err) {
    t.fail(err);
  }
});
(0, _tape.default)("places `global` as first arg following `yarn` when using yarn and `--global` is specified", async t => {
  t.plan(1);

  try {
    const command = await getCliInstallCommand(["eslint-config-airbnb", "--global", "-Y"]);
    t.equal(/^yarn global/.test(command), true);
  } catch (err) {
    t.fail(err);
  }
});
(0, _tape.default)("adds explicit `--global` flag when using `--global` with NPM", async t => {
  t.plan(1);

  try {
    const command = await getCliInstallCommand(["eslint-config-airbnb", "--global"]);
    t.equal(/\bnpm\s--global\b/.test(command), true);
  } catch (err) {
    t.fail(err);
  }
});
(0, _tape.default)("does not add `--save` when using `--global` with NPM", async t => {
  t.plan(1);
...
(0, _tape.default)("adds an explicit `--no-save` when using `--silent` with NPM", async t => {
  t.plan(1);

  try {
    const command = await getCliInstallCommand(["eslint-config-airbnb", "--silent"]);
    t.equal(/\s--no-save\b/.test(command), true);
  } catch (err) {
    t.fail(err);
  }
});
(0, _tape.default)("installs with pnpm successfully", t => {
  ...
  cli.on("exit", code => {
    ...
    const pnpmLockYamlPath = _path.default.resolve(__dirname, "..", "fixtures", "pnpm", "pnpm-lock.yaml");

    const hasPnpmLockYaml = _fs.default.existsSync(pnpmLockYamlPath);

    t.equal(hasPnpmLockYaml, true); // Delete pnpm-lock.yaml file

    _fs.default.unlinkSync(pnpmLockYamlPath);
